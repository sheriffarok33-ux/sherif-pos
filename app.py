...
if not st.session_state["branch_verified"]:
  st.title(f"🔥 أهلاً بك يا بطل العمل، {st.session_state['username']}! استعد ليوم عمل متميز ومشوق.")
...
if choice == "🚪 تسجيل الخروج":
  ...
  # قراءة الإحصائيات الحقيقية لتفعيل البطاقات
  conn = get_db_connection()
  ...
  st.info("📌 **ملاحظة:** البرنامج سيتخطى سطر العناوين الأول تلقائياً.")
    ...
    if up_file and st.button("📥 تنفيذ استيراد الأصناف"):
      try:
        ...
        for idx, row in df.iterrows():
          ...
          # تخطي السطر لو كان يحتوي على كلمات العناوين
        ...
        if success_count > 0:
            st.success(f"🎉 تم استيراد وتوزيع عدد ({success_count}) صنف بنجاح للشركة المحددة!")
        else:
            st.warning("⚠️ لم يتم استيراد أي صنف. يرجى التأكد من أن عمود الصنف (العمود الثاني) ليس فارغاً في ملف الإكسيل.")
