...
@st.dialog("🌟 ترحيب وتنبيهات البضاعة الواردة")
def welcome_user_dialog():
    ...
    if user_role == "Cashier" and user_branch_id:
        pending_invoices = conn.execute("SELECT * FROM transfer_logs WHERE status = 'معلقة' AND to_branch_id = ?", (user_branch_id,)).fetchall()
        if pending_invoices:
            st.warning(f"⚠️ تنبيه هامة: يوجد عدد ({len(pending_invoices)}) فاتورة تزويد بضاعة تم ترحيلها لفرعك من المخزن الرئيسي ويجب عليك الاطلاع عليها واستلامها!")
            for pi in pending_invoices:
                from_b = conn.execute("SELECT branch_name FROM branches WHERE id = ?", (pi["from_branch_id"],)).fetchone()
                b_name = from_b["branch_name"] if from_b else "المخزن"
                st.markdown(f"""
                <div style="background:#fef3c7; padding:10px; border-radius:6px; margin-bottom:6px; border:1px solid #f59e0b;">
                    <b>فاتورة رقم #{pi['id']} من ({b_name})</b><br>
                    <b>التفاصيل:</b> {pi['items_details']}
                </div>
                """, unsafe_allow_html=True)
                if st.button(f"✅ تم الاستلام واعتماد الفاتورة #{pi['id']}", key=f"welcome_acc_{pi['id']}"):
                    conn.execute("UPDATE transfer_logs SET status = 'مكتملة' WHERE id = ?", (pi['id'],))
                    conn.commit()
                    st.success(f"تم استلام الفاتورة #{pi['id']} بنجاح وإضافتها لمخزون الفرع!")
                    st.rerun()
    conn.close()
    
    if st.button("موافق ومتابعة العمل", use_container_width=True, type="primary"):
        st.session_state["show_welcome_dialog"] = False
        st.rerun()

@st.dialog("🔒 تأكيد برقم المدير")
def admin_confirm_dialog(action_type, target_id=None):
    st.warning(f"تأكيد عملية ({action_type}) تتطلب الرقم السري للمدير:")
    admin_pass = st.text_input("الرقم السري:", type="password", key="diag_pass_input")
    
    col1, col2 = st.columns(2)
    with col1:
        if st.button("تنفيذ", use_container_width=True, type="primary"):
            if verify_admin_password(admin_pass):
                ...
                if action_type == "تقرير Z (تصفير مبيعات اليوم)":
                    ...
                    conn.commit()
                    st.session_state["success_alert_msg"] = "تم تصفير مبيعات اليوم (تقرير Z) بنجاح وإظهار التراكمي!"
                elif action_type == "حذف فرع": conn.execute("DELETE FROM branches WHERE id = ?", (target_id,))
...
def checkout_payment_dialog(b_id, g_tot):
    ...
    if matching_supplier:
        st.info(f"💡 هذا الزبون مسجل كمورد أيضاً! (رقم المورد: {matching_supplier['id']} - رصيد مستحقاته لدينا: {matching_supplier['balance']:,.2f} د.ل)")
...
@st.dialog("🔍 بيع صنف يدوي (بدون باركود)")
def manual_item_dialog():
    st.subheader("إضافة صنف حر / يدوي للسلة")
    m_name = st.text_input("اسم الصنف:")
    m_price = st.number_input("سعر البيع (د.ل):", min_value=0.0, value=0.0, step=0.5)
    m_qty = st.number_input("الكمية (كجم):", min_value=0.1, value=1.0, step=0.1)
    
    if st.button("إضافة للسلة", use_container_width=True, type="primary"):
        if m_name.strip() and m_price > 0:
            st.session_state["cart"].append({
                "id": 88888, "code": "MANUAL", "name": m_name.strip(),
                "price": float(m_price), "qty": float(m_qty), "total": float(m_price) * float(m_qty)
            })
            st.session_state["success_alert_msg"] = "تمت إضافة الصنف اليدوي للسلة!"
            st.rerun()
        else:
            st.error("يرجى إدخال اسم الصنف وسعر صحيح.")
...
def shift_report_x_dialog():
    st.subheader("تقرير الشفت (X) - حركة المبيعات الحالية")
    conn = get_db_connection()
    total_sales = conn.execute("SELECT SUM(total_amount) FROM invoices").fetchone()[0] or 0.0
    inv_count = conn.execute("SELECT COUNT(*) FROM invoices").fetchone()[0] or 0
    st.metric("إجمالي مبيعات الشفت الحالي", f"{total_sales:,.2f} د.ل")
    st.metric("عدد الفواتير المصدرة", f"{inv_count}")
    conn.close()
    if st.button("إغلاق"):
        st.rerun()
...
if choice == "🏠 الرئيسية واللوحة":
  ...
  tab_tr1, tab_tr2 = st.tabs(["تزويد فرع", "أرشيف التزويد"])
  with tab_tr1:
      if main_store:
          ...
          if st.session_state["transfer_cart"]:
              ...
              if st.button("ترحيل وإرسال بضاعة للفرع", type="primary"):
                  cur_tr = conn.cursor()
                  details = []
                  ...
                  st.session_state["success_alert_msg"] = "تم ترحيل بضاعة التزويد بنجاح وستظهر فوراً كرسالة تنبيه للكاشير بالفرع!"
                  st.rerun()
  with tab_tr2:
  ...
  with tab_p1:
      b_dict = {b["branch_name"]: b["id"] for b in conn.execute("SELECT id, branch_name FROM branches").fetchall()}
      suppliers_list = conn.execute("SELECT id, supplier_name, phone FROM suppliers").fetchall()
      
      pb = st.selectbox("الفرع:", list(b_dict.keys()))
      
      if suppliers_list:
          sup_opts = {f"رقم [{s['id']}] - {s['supplier_name']} (هاتف: {s['phone']})": s["id"] for s in suppliers_list}
          sel_sup_label = st.selectbox("اختر المورد من القائمة:", list(sup_opts.keys()))
          sup_id = sup_opts[sel_sup_label]
          selected_sup_row = conn.execute("SELECT supplier_name FROM suppliers WHERE id = ?", (sup_id,)).fetchone()
          ps = selected_sup_row["supplier_name"] if selected_sup_row else ""
      else:
          ps = ""
          st.warning("لا توجد موردين مسجلين. يرجى إضافتهم من تبويب (الموردين والديون) أولاً.")
  ...
  with tab_p2:
      with st.form("new_sup", clear_on_submit=True):
          ...
          if st.form_submit_button("حفظ المورد") and sname:
              try:
                  ...
                  st.rerun()
              except: st.error("خطأ في حفظ المورد.")
      
      supp_df = pd.read_sql("SELECT id AS 'رقم المورد', supplier_name AS 'المورد', phone AS 'الهاتف', balance AS 'الديون والمستحقات (د.ل)' FROM suppliers", conn)
      if not supp_df.empty: 
          ...
          del_s = st.selectbox("المورد للحذف:", supp_df["رقم المورد"].tolist(), format_func=lambda x: f"رقم: {x} - {supp_df[supp_df['رقم المورد']==x]['المورد'].values[0]}")
  ...
  with col_btn3:
      if st.button("📊 تقرير الشفت (X)", use_container_width=True):
          shift_report_x_dialog()
  with col_btn4:
      if st.button("🔒 تقرير وتصفير (Z)", use_container_width=True):
          admin_confirm_dialog("تقرير Z (تصفير مبيعات اليوم)")

  st.markdown("---")
  b_id = user_branch_id if user_branch_id else 1
  
  st.text_input("مسح الباركود السريع:", key="unified_barcode", on_change=process_unified_barcode, placeholder="امسح الباركود هنا واضغط Enter...")

  col_g, col_c = st.columns([2, 1])
  with col_g:
      st.markdown("### 🔍 البحث السريع عن الأصناف (بدون باركود)")
      all_branch_items = conn.execute("SELECT * FROM items WHERE branch_id = ?", (b_id,)).fetchall()
      
      if all_branch_items:
          item_names_dict = {f"[{i['item_code']}] {i['item_name']} (متاح: {i['quantity']} كجم - السعر: {i['sale_price']} د.ل)": i for i in all_branch_items}
          selected_item_label = st.selectbox("ابحث عن اسم الصنف أو اختره من القائمة:", ["-- اختر الصنف --"] + list(item_names_dict.keys()))
          
          if selected_item_label != "-- اختر الصنف --":
              chosen_item = item_names_dict[selected_item_label]
              with st.form(key=f"search_add_form_{chosen_item['id']}", clear_on_submit=True):
                  qty_search = st.number_input("الكمية المطلوبة (كجم):", min_value=0.1, value=1.0, step=0.1, format="%.2f")
                  if st.form_submit_button("➕ إضافة للسلة", use_container_width=True):
                      if qty_search > 0:
                          st.session_state["cart"].append({
                              "id": chosen_item["id"], "name": chosen_item["item_name"], 
                              "price": float(chosen_item["sale_price"]), "qty": float(qty_search), 
                              "total": float(chosen_item["sale_price"]) * float(qty_search)
                          })
                          st.rerun()
      else:
          st.info("لا توجد أصناف مسجلة بالفرع حالياً.")
