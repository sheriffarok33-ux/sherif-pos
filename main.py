...
# تنسيق CSS لضبط الاتجاه والخطوط
st.markdown("""
    <style>
...
# ==========================================
# 3. الاستدعاء المباشر للشاشات (بدون إخفاء الأخطاء)
# ==========================================
import users
import pos
import transfers
...
# التحقق من وجود ملف الفروع لتفادي توقف النظام في حال حذفه
try:
    import branches
except ImportError:
    branches = None
    ...
    for menu_name in DEFAULT_MENUS:
        if check_user_permission(menu_name):
            if st.session_state["page"] == menu_name:
                st.button(f"📍 {menu_name}", use_container_width=True, key=f"btn_active_{menu_name}")
            else:
                if st.button(menu_name, use_container_width=True, key=f"btn_{menu_name}"):
                    set_page(menu_name)
...
if choice == "🛒 نقطة البيع (POS)":
    ...
    else: st.warning("ملف إدارة الفروع مفقود.")
