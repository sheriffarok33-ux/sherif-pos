...
def show_page():
    st.markdown("""
        ...
        .dash-card h3 { color: #94a3b8 !important; font-size: 16px !important; margin-bottom: 8px; }
        .dash-card h2 { color: #38bdf8 !important; font-size: 26px !important; font-weight: 900 !important; margin: 0; }
    ...
    st.info("💡 مرحباً بك في النظام السحابي لإدارة المحامص والمخازن.")
    st.markdown("---")

    try:
        conn = get_db_connection()
        
        # جلب الإحصائيات مع الحماية من الجداول الفارغة
        try:
        ...
        with col2:
            st.markdown(f'<div class="dash-card"><h3>🏢 الفروع والمخازن</h3><h2>{branches_count} فرع</h2></div>', unsafe_allow_html=True)
        with col3:
            st.markdown(f'<div class="dash-card"><h3>📦 إجمالي المخزون</h3><h2>{total_items_qty:,.1f}</h2></div>', unsafe_allow_html=True)
        with col4:
            st.markdown(f'<div class="dash-card"><h3>👥 طاقم العمل</h3><h2>{users_count} موظف</h2></div>', unsafe_allow_html=True)
        ...
        try:
            ...
            st.info("📭 جدول الفواتير بانتظار العمليات الأولى.")

        conn.close()
    except Exception as db_err:
        st.error(f"⚠️ تنبيه في قاعدة البيانات: {db_err}")
